...
// Root1, Root2, ArmA1, ArmA2, ArmB, WristA, WristB
uint8_t servoChannel[NUM_SERVOS] = {11, 12, 1, 2, 3, 5, 7};
...
void loop()
{
  ...
  {
    // Root (dual servos)
    writeServoAngle(servoChannel[0], angleValues[0]);
    writeServoAngle(servoChannel[1], angleValues[0]);

    // Arm A
    writeServoAngle(servoChannel[2], angleValues[1]);
    writeServoAngle(servoChannel[3], 180 - angleValues[1]);

    // Arm B
    writeServoAngle(servoChannel[4], angleValues[2]);

    // Wrist A
    writeServoAngle(servoChannel[5], angleValues[3]);

    // Wrist B
    writeServoAngle(servoChannel[6], angleValues[4]);
  }
}